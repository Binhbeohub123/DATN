...
public class DatVe {
    ...
    // ── Staff & Check-in fields (Phase 1 migration) ──

    /**
     * Staff member who created a counter sale (nullable — null for online bookings)
     */
    @ManyToOne(fetch = FetchType.LAZY)
    ...
    /** Check-in status: 'chưa sử dụng' | 'đã sử dụng' */
    @Column(name = "TrangThaiCheckIn", length = 20)
    private String trangThaiCheckIn;

    /** Timestamp when ticket was scanned / checked in */
    @Column(name = "ThoiGianCheckIn")
    private LocalDateTime thoiGianCheckIn;

    /**
     * Staff member who scanned the ticket at check-in (may differ from NhanVien who
     * sold it)
     */
    @ManyToOne(fetch = FetchType.LAZY)
    ...
    protected void onCreate() {
        ...
        if (tienGiamKhuyenMai == null)
            tienGiamKhuyenMai = BigDecimal.ZERO;
        if (diemSuDung == null)
            diemSuDung = 0;
        if (tienGiamTuDiem == null)
        ...
        if (trangThaiCheckIn == null)
            trangThaiCheckIn = "chưa sử dụng";
    }
}