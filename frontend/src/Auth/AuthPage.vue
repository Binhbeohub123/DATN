    ...
    <div class="auth-card glass-card--heavy">
      ...
      <div class="right">
        <div class="form-box">
          ...
          <!-- ================= ACCOUNT LOCKED NOTICE (OAuth redirect) ================= -->
          <transition name="slide">
            <div v-if="route.query.locked === 'true' && !error && !success" class="msg warn glass-card">
              <span class="icon">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" width="16" height="16"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>
              </span>
              <span class="text">
                Tài khoản của bạn đã bị khóa{{ route.query.reason ? ' — Lý do: ' + route.query.reason : '' }}. Vui lòng liên hệ quản trị viên để được mở khóa.
              </span>
            </div>
          </transition>
...
// ================= STATE =================
// isRegister starts true when ?mode=register is in the URL — so /auth?mode=register
// opens directly on the Register form without requiring a manual tab click.
...
const isLoading = ref({
  ...
  forgot: false, reset: false, google: false,
})
  ...
  try {
    ...
    setTimeout(() => {
      ...
      if (intended && intended !== '/auth') {
        ...
        const dest = authStore.userRole === 'ADMIN' ? '/admin'
                   : authStore.userRole === 'STAFF' ? '/staff/dashboard'
                   : '/'
        router.push(dest)
      }
    }, 600)
...
function getBackendOrigin() {
  ...
  // Local dev with Vite proxy (VITE_API_BASE_URL="/api") or unset:
  // must use full backend URL for OAuth2
